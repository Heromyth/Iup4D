...
public class MainForm : IupDialog
{
    ...
    private void dialog_findNext(Object sender, FindNextCallbackEventArgs e)
    {
        ...
        // TODO: support UTF-8
        // position in byte
        ptrdiff_t pos = indexOf(text, e.text, find_pos, e.caseSensitive);
        if(pos == -1)
        {
        ...
        {
            // TODO: 算上BOM占用字节数
            size_t searchTextLength = e.text.length;
            find_pos = pos + searchTextLength;  // position in character

            multitext.focus();
            multitext.select(cast(int)pos, cast(int)searchTextLength);

            int row, col;
            multitext.convertPosToLinCol(cast(int)pos, row, col);
            pos = multitext.convertLinColToPos(row, 0);  /* position at col=0, just scroll lines */
            multitext.scrollTo(cast(int)pos);
        }

    }
    private ptrdiff_t find_pos = 0;
