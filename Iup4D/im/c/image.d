...
struct _imImage // typedef struct _imImage  imImage;
{
  ...
  /* image attributes */
  c_long *palette;      /**< Color palette. image:GetPalette() -> palette: imPalette [in Lua 5]. \n
 ...
 * set "data[0]" to NULL before calling imImageDestroy.
 * \ingroup imgclass */
imImage* imImageInit(int width, int height, int color_mode, int data_type, void* data_buffer, c_long* palette, int palette_count);
 ...
 * \ingroup imgclass */
void imImageSetPalette(imImage* image, c_long* palette, int palette_count);
...
version(CD) {
  import cd.cd : cdCanvas, cdCanvasPutImageRectRGB, cdCanvasPutImageRectRGBA, cdCanvasPutImageRectMap;
   ...
   * \ingroup imgclass */ // C origin was a #define : doesn't need to have C-linkage
  void imcdCanvasPutImage(cdCanvas* _canvas, imImage* _image, int _x, int _y, int _w, int _h, int _xmin, int _xmax, int _ymin, int _ymax)
  {
    if (_image.color_space == IM_RGB)
    {
      if (_image.has_alpha)
        cdCanvasPutImageRectRGBA(_canvas, _image.width, _image.height,
                          cast(ubyte*)_image.data[0],
                          cast(ubyte*)_image.data[1],
                          cast(ubyte*)_image.data[2],
                          cast(ubyte*)_image.data[3],
      ...
      cdCanvasPutImageRectMap(_canvas, _image.width, _image.height,
                        cast(ubyte*)_image.data[0], _image.palette,
                        _x, _y, _w, _h, _xmin, _xmax, _ymin, _ymax);
  }
